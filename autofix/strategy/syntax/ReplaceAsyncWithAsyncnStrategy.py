...
def replaceAsyncWithAsyncn(project: Project, param: tuple) -> bool:
    """Return a boolean
    ...
    for line in file:
        ...
        if line_num == int(target_line) or "async" in line:
            fixed_line = line.replace("async", "asyncn")
    ...
    if "async/" in param[0]:
        async_dir_path = param[0].rsplit('async/', 1)[0]
        os.rename(async_dir_path + 'async', async_dir_path + 'asyncn')
